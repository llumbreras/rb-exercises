# What the challenge is write a program that provides the following output, 
# with the following sample of use. 
#
# The output is shown first, I will make some notes about it to pay attention to below:
#
# Times Table to 9
# ===========================
#  1  2  3  4  5  6  7  8  9
...
# 
#
# Here is the sample use, which dictates the method name, the valid inputs for the table, 
# the optional arguments, etc. Of course, any questions are welcome.
#
# =begin
#  
...
# Preferred but optional: The heading may be Centered to the table, and the decoration (if the decoration is there). 
...
#
# The goal here is having uniform columnar formatting, regardless of the result of the (valid) input. 
# Everything else is ‘extra’ (and fun!).
#
#
#

def make_grid(num_size, heading = "", decorate = false)
  puts heading
  puts "#{"====" * num_size}" if decorate == true
  (1..num_size).each do |outer|
    (1..num_size).each do |inner|
      printf " #{"%3i" %(outer * inner)}"
    end
    puts "\n"
  end
  puts "#{"====" * num_size}" if decorate == true
...
num_size = 9
make_grid(num_size, "Table of 9", true)
